...
class drrnn(nn.Module):
    ...
    def __init__(self, in_N, m, out_N, depth=5):
...
def main():
    ...
    for epoch in range(epochs+1):
        ...
        lapla = autograd.grad(outputs=grads, inputs=xr,
                              grad_outputs=torch.ones_like(grads),
                              create_graph=True, retain_graph=True, only_inputs=True)[0]

        loss_r = lapla + 1
        loss_r = torch.mean(torch.pow(loss_r, 2))
        ...
        if epoch > int(2 * epochs / 3):
            if torch.abs(loss) < best_loss:
                best_loss = loss.item()
                best_epoch = epoch
                torch.save(model.state_dict(), 'best_ls.mdl')
    print('best epoch:', best_epoch, 'best loss:', best_loss)


    # plot figure
    model.load_state_dict(torch.load('best_ls.mdl'))
    print('load from ckpt!')
    with torch.no_grad():
        x1 = torch.linspace(-1, 1, 101)
